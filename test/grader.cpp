...
int main(int argc, char* argv[]){
    ...
    if(_lower_bound({}, 0) == INT_MAX && _upper_bound({}, 0) == INT_MAX){
        ...
        assert("Github Autograder Mode ON!" && argc == 1);
        return 0;
    }
    ...
    for(vector<int> test: testSet){
        ...
        trueOut = upper_bound(test.begin(), test.end(), test[randomInd]) - test.begin();
        totalTests += 1;
        totalPassed += (testOut == trueOut);
        ...
        trueOut = upper_bound(test.begin(), test.end(), randomNum) - test.begin();
        totalTests += 1;
        totalPassed += (
            binary_search(test.begin(), test.end(), randomNum) 
            ? 
